...
async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    try {
      ...
      switch (action) {
        case 'publish':
          result = await Project.updateMany({ _id: { $in: projectIds } }, { $set: { published: true } });
          auditAction = 'bulk-publish';
          break;
        case 'draft':
          result = await Project.updateMany({ _id: { $in: projectIds } }, { $set: { published: false } });
      ...
      res.status(200).json({ success: true, message: `Successfully performed ${action} on ${result.deletedCount || result.modifiedCount} projects.` });
    } catch (error) {
      console.error(`Failed to perform bulk action '${action}' on projects:`, error);
