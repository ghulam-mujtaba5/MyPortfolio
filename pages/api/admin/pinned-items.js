...
async function handler(req, res) {
  ...
  try {
    const pinnedArticles = await Article.find({ pinned: true }).sort({ updatedAt: -1 }).limit(5);
    const pinnedProjects = await Project.find({ pinned: true }).sort({ updatedAt: -1 }).limit(5);
