...
const MOCK_RESPONSES = {
  formal: (text) => `It is with great consideration that we address the following matter: ${text}. We anticipate a positive outcome.`,
  casual: (text) => `Hey, so here's the deal: ${text}. Should be pretty cool.`,
  confident: (text) => `Listen up. ${text}. We will achieve this, period.`,
  friendly: (text) => `Hi there! Just wanted to share a thought: ${text}. Hope you find it helpful!`,
  professional: (text) => `Regarding the subject matter, the following points are pertinent: ${text}. All stakeholders are advised to review.`,
};
