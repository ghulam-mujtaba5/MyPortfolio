  ...
  switch (method) {
    case 'GET':
      try {
        ...
        const versions = await ArticleVersion.find({ articleId }).sort({ createdAt: -1 }).limit(20);
        res.status(200).json({ success: true, data: versions });
      } catch (error) {
        res.status(400).json({ success: false, message: 'Failed to fetch versions' });
      }
      break;
    ...
    case 'POST':
      try {
        ...
        res.status(400).json({ success: false, message: 'Failed to save version', errors: error.errors });
      }
      break;
    ...
    case 'DELETE':
        try {
            ...
            res.status(400).json({ success: false, message: 'Failed to delete version' });
        }
        break;
