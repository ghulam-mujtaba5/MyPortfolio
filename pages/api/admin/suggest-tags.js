...
const generateMockTags = async (content) => {
  ...
  if (lowerContent.includes('next.js') || lowerContent.includes('nextjs')) tags.add('Next.js');
  if (lowerContent.includes('node.js') || lowerContent.includes('nodejs')) tags.add('Node.js');
  ...
  if (lowerContent.includes('database') || lowerContent.includes('mongodb')) tags.add('Database');
  if (lowerContent.includes('deployment') || lowerContent.includes('vercel')) tags.add('Deployment');
  if (lowerContent.includes('testing') || lowerContent.includes('jest')) tags.add('Testing');
  if (lowerContent.includes('ai') || lowerContent.includes('artificial intelligence')) tags.add('AI');
...
const handler = async (req, res) => {
  ...
  if (!content || content.trim().length < 50) {
    return res.status(400).json({ success: false, message: 'Content must be at least 50 characters long.' });
  }
  ...
  try {
    ...
    res.status(500).json({ success: false, message: 'Failed to generate tags.' });
  }
};
