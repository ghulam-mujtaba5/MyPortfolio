...
const ProjectPage = ({ project, preview }) => {
...
export async function getServerSideProps(context) {
  ...
  if (preview && previewData?.id) {
    // For preview mode, fetch by ID, regardless of published status
    project = await Project.findById(previewData.id).lean();
  } else {
    // For public view, fetch by slug and ensure it's published
    project = await Project.findOne({ slug: params.slug, published: true }).lean();
  }
