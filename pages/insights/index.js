...
export default function InsightsPage({
  ...
  // Client-side fallback when SSR returns nothing
  useEffect(() => {
    if (!mounted) return;
  ...
  useEffect(() => {
    const id = setTimeout(() => {
      if (q !== search) {
        ...
        newQuery.page = 1;
        router.push({ pathname: "/insights", query: newQuery });
      }
    }, 400);
  ...
  }, [articles, selectedCategory, mapToBucket]);
  ...
  return (
    <>
      <SEO
        title="Insights | Ghulam Mujtaba — Software, Data Science & AI"
        description="Insights and articles by Ghulam Mujtaba on software engineering, data science, machine learning, AI, and modern web development."
        url="https://ghulammujtaba.com/insights"
        canonical="https://ghulammujtaba.com/insights"
        image="https://ghulammujtaba.com/og-image.png"
        imageWidth={1200}
        imageHeight={630}
        imageAlt="Insights by Ghulam Mujtaba"
        author="Ghulam Mujtaba"
        keywords="Insights, Blog, Ghulam Mujtaba, Software, Data Science, AI, Machine Learning, Web Development"
        jsonLd={[
          collectionPageSchema({
            name: "Insights by Ghulam Mujtaba",
            url: "https://ghulammujtaba.com/insights",
            description: "Insights and articles by Ghulam Mujtaba on software engineering, data science, machine learning, and AI.",
            items: (articles || []).slice(0, 10).map((a) => ({
              name: a.title,
              url: `https://ghulammujtaba.com/insights/${a.slug}`,
      ...
      <div
        className={`insights-page-bg ${theme}`}
        style={{
          display: "flex",
      ...
      >
        ...
        >
          ...
          {!loading && !error && (
            <div className="insight-tags fade-in-up">
              {CATEGORY_FILTERS.map((cat, i) => (
                <button
                  key={cat}
                  className={`insight-tag-btn${selectedCategory === cat ? " active" : ""}`}
          ...
          {!mounted && initialArticles.length === 0 && (
            <div className={listCss.grid}>
              {Array.from({ length: 6 }).map((_, i) => (
                <div className={listCss.skeletonCard} key={`init-skeleton-${i}`}>
                  <div className={listCss.skelImage} />
                  <div className={listCss.skelContent}>
                    <div className={`${listCss.skelLine} ${listCss.medium}`} />
                    <div className={`${listCss.skelLine} ${listCss.medium}`} />
          ...
          {!loading && !error && mounted && filteredArticles.length === 0 && (
            <div className={listCss.empty}>
              {(() => {
                if (search && tag) return `No insights found for "${search}" with the tag "${tag}".`;
                if (search) return `No insights found for "${search}". Try a different search.`;
                if (tag) return `No insights found with the tag "${tag}".`;
                if (selectedCategory && selectedCategory !== "All")
                  return `No insights found in the category "${selectedCategory}".`;
                return "No insights yet. Check back soon.";
              })()}
            </div>
          )}
          ...
          {!loading && !error && filteredArticles.length > 0 && (
            <div className={listCss.grid}>
              {filteredArticles.map((a, index) => (
                <ScrollReveal key={a._id} animation="fadeInUp" delay={index * 50} width="100%">
          ...
          {!loading && !error && pagination && pagination.totalPages > 1 && (
            <div className={listCss.pagination}>
              <button
                ...
                disabled={page <= 1}
                className={`${listCss.pageBtn} ${theme === "dark" ? darkCss.pageBtn : lightCss.pageBtn}`}
              >
                Previous
              </button>
              <span className={`${listCss.pageInfo} ${theme === "dark" ? darkCss.pageInfo : lightCss.pageInfo}`}>
              ...
              <button
                onClick={() =>
                  router.push({
                    pathname: "/insights",
                    query: { ...router.query, page: Math.min(pagination.totalPages || page + 1, page + 1) },
                  })
                }
      ...
      <style jsx>{`
        .insights-page-bg.dark .insight-tag-btn {
          background: #23272f;
          color: #eee;
          border-color: #23272f;
        }
        .insights-page-bg.dark .insight-tag-btn.active,
        .insights-page-bg.dark .insight-tag-btn:hover {
          background: #fff;
          color: #22223b;
          border-color: #fff;
        }
  ...
  try {
    ...
    if (category) {
      ...
      if (bucket !== "all" && bucket !== "others") {
        ...
        if (regexes.length > 0) baseFilter.categories = { $in: regexes };
      }
    }
    ...
    console.error("[Insights SSR] getServerSideProps failed:", e?.message || e);
    return {
      props: {
        initialArticles: [],
        initialPagination: { page: 1, totalPages: 1, total: 0, limit: 9 },
