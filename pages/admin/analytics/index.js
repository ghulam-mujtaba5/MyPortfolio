...
const AnalyticsPage = () => {
  ...
  const computeDeltaPct = (series, windowSize = 7) => {
    ...
    const values = series.map(d => (typeof d === 'number' ? d : d?.count || 0));
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const response = await fetch(`/api/admin/analytics${range ? `?range=${range}` : ''}`);
  ...
  const processChartData = () => {
    ...
    return { articleStatusData, projectStatusData, articlesTrendData, projectsTrendData };
  };

  const { articleStatusData, projectStatusData, articlesTrendData, projectsTrendData } = processChartData();
  ...
  return (
    <AdminLayout title="Analytics">
      ...
      <div style={{display:'grid', gridTemplateColumns:'repeat(4, minmax(0, 1fr))', gap:'1rem', margin:'0.75rem 0'}}>
        <div style={{padding:'12px', border:'1px solid #e5e7eb', borderRadius:8}}>
          ...
          <div style={{display:'flex', alignItems:'baseline', gap:8}}>
            <div style={{fontSize:22, fontWeight:600}}>{stats?.kpis?.totalArticles ?? '-'}</div>
            {(() => {
              ...
              const sign = pos ? '+' : '';
              return <span style={{fontSize:12, color}}>{sign}{delta.toFixed(1)}% vs prev 7d</span>;
            })()}
          </div>
          <Sparklines data={(stats?.articlesByDate||[]).map(d=>d.count)} height={30} margin={5}>
            <SparklinesLine color="#4f46e5" style={{ fill: 'none' }} />
            <SparklinesSpots size={2} />
            <SparklinesReferenceLine type="mean" style={{ stroke: '#9ca3af', strokeOpacity: 0.6, strokeDasharray: '2,2' }} />
          </Sparklines>
        </div>
        <div style={{padding:'12px', border:'1px solid #e5e7eb', borderRadius:8}}>
          <div style={{fontSize:12, opacity:0.7}}>Published Articles</div>
          <div style={{display:'flex', alignItems:'baseline', gap:8}}>
            <div style={{fontSize:22, fontWeight:600}}>{stats?.kpis?.publishedArticles ?? '-'}</div>
            {(() => {
              const delta = computeDeltaPct(stats?.articlesByDate || [], 7);
              if (delta === null) return null;
              const pos = delta >= 0;
        ...
        <div style={{padding:'12px', border:'1px solid #e5e7eb', borderRadius:8}}>
          ...
          <div style={{display:'flex', alignItems:'baseline', gap:8}}>
            <div style={{fontSize:22, fontWeight:600}}>{stats?.kpis?.totalProjects ?? '-'}</div>
            {(() => {
          ...
          <Sparklines data={(stats?.projectsByDate||[]).map(d=>d.count)} height={30} margin={5}>
        ...
        <div style={{padding:'12px', border:'1px solid #e5e7eb', borderRadius:8}}>
          ...
          <div style={{display:'flex', alignItems:'baseline', gap:8}}>
            <div style={{fontSize:22, fontWeight:600}}>{stats?.kpis?.publishedProjects ?? '-'}</div>
            {(() => {
              const delta = computeDeltaPct(stats?.projectsByDate || [], 7);
              if (delta === null) return null;
              const pos = delta >= 0;
      ...
      <div style={{display:'flex', gap:8, alignItems:'center', margin:'8px 0 12px'}}>
        ...
        <button
          ...
          onClick={() => {
            ...
            pushRows((stats.projectCategories||[]).map(c => [c._id||'Uncategorized', c.count]));
            rows.push([]);

            // Top/Recent
            pushSection('Top Viewed Articles');
            pushHeader(['Title','Slug','Views','Created At','Id']);
            pushRows((stats.topViewedArticles||[]).map(a => [a.title, a.slug, a.views, a.createdAt, a._id]));
            rows.push([]);
            pushSection('Top Viewed Projects');
            pushHeader(['Title','Slug','Views','Created At','Id']);
            pushRows((stats.topViewedProjects||[]).map(p => [p.title, p.slug, p.views, p.createdAt, p._id]));
            rows.push([]);
            pushSection('Recent Articles');
            pushHeader(['Title','Slug','Published','Created At','Id']);
            pushRows((stats.recentArticles||[]).map(a => [a.title, a.slug, a.published, a.createdAt, a._id]));
            rows.push([]);
            pushSection('Recent Projects');
            pushHeader(['Title','Slug','Published','Created At','Id']);
            pushRows((stats.recentProjects||[]).map(p => [p.title, p.slug, p.published, p.createdAt, p._id]));
          ...
          style={{marginLeft:'auto', padding:'6px 10px', border:'1px solid #e5e7eb', borderRadius:6}}
        >Export CSV</button>
      </div>
      {/* KPI Tiles */}
      {stats?.kpis && (
        <div style={{display:'grid', gridTemplateColumns:'repeat(4, minmax(0, 1fr))', gap:'1rem', margin:'1rem 0'}}>
          <div style={{padding:'1rem', border:'1px solid #e5e7eb', borderRadius:8}}>
            <div style={{fontSize:12, opacity:0.7}}>Total Articles</div>
            <div style={{fontSize:24, fontWeight:700}}>{stats.kpis.totalArticles}</div>
          </div>
          <div style={{padding:'1rem', border:'1px solid #e5e7eb', borderRadius:8}}>
            <div style={{fontSize:12, opacity:0.7}}>Published Articles</div>
            <div style={{fontSize:24, fontWeight:700}}>{stats.kpis.publishedArticles}</div>
          </div>
          <div style={{padding:'1rem', border:'1px solid #e5e7eb', borderRadius:8}}>
            <div style={{fontSize:12, opacity:0.7}}>Total Projects</div>
            <div style={{fontSize:24, fontWeight:700}}>{stats.kpis.totalProjects}</div>
          </div>
          <div style={{padding:'1rem', border:'1px solid #e5e7eb', borderRadius:8}}>
            <div style={{fontSize:12, opacity:0.7}}>Published Projects</div>
            <div style={{fontSize:24, fontWeight:700}}>{stats.kpis.publishedProjects}</div>
          </div>
        </div>
      )}
        ...
        <div style={{padding:'1rem', border:'1px solid #e5e7eb', borderRadius:8}}>
          ...
          <div style={{display:'flex', flexWrap:'wrap', gap:8}}>
            {stats?.articleTags?.map(t => (
              <span
                key={t._id}
                onClick={() => router.push(`/articles?tag=${encodeURIComponent(t._id)}`)}
                title="View articles with this tag"
                style={{padding:'4px 10px', border:'1px solid #e5e7eb', borderRadius:999, fontSize:12, cursor:'pointer'}}
              >
                {t._id} ({t.count})
              </span>
            ))}
        ...
        <div style={{padding:'1rem', border:'1px solid #e5e7eb', borderRadius:8}}>
          ...
          <div style={{display:'flex', flexWrap:'wrap', gap:8}}>
            {stats?.projectTags?.map(t => (
              <span
                key={t._id}
                onClick={() => router.push(`/projects?tag=${encodeURIComponent(t._id)}`)}
        ...
        <div style={{padding:'1rem', border:'1px solid #e5e7eb', borderRadius:8}}>
          ...
          <ul style={{margin:0, paddingLeft:'1rem'}}>
            {stats?.projectCategories?.map(c => (
              <li
                key={c._id}
                onClick={() => router.push(`/projects?category=${encodeURIComponent(c._id||'')}`)}
            ...
            <div>
              ...
              <ul style={{margin:0, paddingLeft:'1rem'}}>
                {stats?.topViewedArticles?.map(a => (
                  <li key={a.slug} style={{cursor:'pointer'}} onClick={() => router.push(`/admin/articles/edit/${a._id}`)}>
            ...
            <div>
              ...
              <ul style={{margin:0, paddingLeft:'1rem'}}>
                {stats?.topViewedProjects?.map(p => (
                  <li key={p.slug} style={{cursor:'pointer'}} onClick={() => router.push(`/admin/projects/edit/${p._id}`)}>
            ...
            <div>
              ...
              <ul style={{margin:0, paddingLeft:'1rem'}}>
                {stats?.recentArticles?.map(a => (
                  <li key={a.slug} style={{cursor:'pointer'}} onClick={() => router.push(`/admin/articles/edit/${a._id}`)}>
                    {a.title} — {new Date(a.createdAt).toLocaleString()} {a.published ? '(Published)' : '(Draft)'}
                  </li>
                ))}
              </ul>
            </div>
            <div>
              ...
              <ul style={{margin:0, paddingLeft:'1rem'}}>
                {stats?.recentProjects?.map(p => (
                  <li key={p.slug} style={{cursor:'pointer'}} onClick={() => router.push(`/admin/projects/edit/${p._id}`)}>
                    {p.title} — {new Date(p.createdAt).toLocaleString()} {p.published ? '(Published)' : '(Draft)'}
                  </li>
                ))}
              </ul>
            </div>
