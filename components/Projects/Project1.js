...
const ProjectCard = React.memo(({ project, frameStyles, theme }) => {
  return (
    <article className={`${commonStyles.projectCard1} ${frameStyles.projectCard1}`} role="article" aria-labelledby={`project-title-${project.title}`}>
      <div className={`${commonStyles.projectCard1Child} ${frameStyles.projectCard1Child}`} />
      <a className={`${commonStyles.livePreview} ${frameStyles.livePreview}`} href={project.livePreviewLink} target="_blank" rel="noopener noreferrer" aria-label={`Live Preview of ${project.title}`}>Live Preview</a>
      <a className={`${commonStyles.viewCode} ${frameStyles.viewCode}`} href={project.viewCodeLink} target="_blank" rel="noopener noreferrer" aria-label={`View Code of ${project.title}`}>View Code</a>
      <img className={`${commonStyles.projectImg1} ${frameStyles.projectImg1}`} alt={`${project.title} screenshot`} src={project.imgSrc} loading="lazy" />
      <h3 className={`${commonStyles.projectTileGoes} ${frameStyles.projectTileGoes}`} id={`project-title-${project.title}`}>{project.title}</h3>
      <div className={`${commonStyles.thisIsSample} ${frameStyles.thisIsSample}`}>{project.description}</div>
      ...
      <img className={`${commonStyles.githubIcon} ${frameStyles.githubIcon}`} alt="GitHub icon" src={theme === 'dark' ? 'GithubDark.svg' : 'github_icon.svg'} loading="lazy" />
      <img className={`${commonStyles.previewIcon1} ${frameStyles.previewIcon1}`} alt="Preview icon" src={theme === 'dark' ? 'PreviewDark.svg' : 'preview_icon1.svg'} loading="lazy" />
    </article>
  );
});
...
const Frame = () => {
  ...
  return (
    <div style={{ margin: '0 6%' }}> {/* Adjust margin as needed */}
