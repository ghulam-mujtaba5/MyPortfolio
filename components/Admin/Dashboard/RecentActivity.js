...
export default function RecentActivity() {
  ...
  if (loading) {
    return <div className={`${commonStyles.container} ${themeStyles.container}`}>Loading activity...</div>;
  }

  return (
    <div className={`${commonStyles.container} ${themeStyles.container}`}>
      <h2 className={`${commonStyles.title} ${themeStyles.title}`}>Recent Activity</h2>
      <ul className={commonStyles.activityList}>
        {activity.map((log) => (
          <li key={log._id} className={`${commonStyles.activityItem} ${themeStyles.activityItem}`}>
            <ActionIcon action={log.action} />
            <div className={commonStyles.activityDetails}>
              <p className={commonStyles.activityText}>
                <strong>{log.userName}</strong> {log.action.replace('_', ' ')}d a(n) <strong>{log.entity}</strong>.
              </p>
              <p className={`${commonStyles.activityTime} ${themeStyles.activityTime}`}>
