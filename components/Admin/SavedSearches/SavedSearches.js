...
const SavedSearches = ({ scope }) => {
    ...
    const handleDeleteSearch = async (id) => {
        try {
            const res = await fetch(`/api/admin/saved-searches?id=${id}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    return (
        <div className={styles.container} ref={dropdownRef}>
            <button onClick={() => setIsDropdownOpen(!isDropdownOpen)} className={styles.dropdownButton}>
                Saved Searches <Icon name={isDropdownOpen ? 'chevron-up' : 'chevron-down'} />
            </button>
            {isDropdownOpen && (
                <div className={styles.dropdownContent}>
                    <div className={styles.saveSection}>
                        ...
                        />
                        <button onClick={handleSaveSearch} className={styles.saveButton}>Save</button>
                    </div>
                    <ul className={styles.searchList}>
                        {savedSearches.length > 0 ? (
                            savedSearches.map(search => (
                                <li key={search._id} className={styles.searchItem}>
                                    <span onClick={() => applySearch(search.query)} className={styles.searchName}>
                                        {search.name}
                                    </span>
                                    <button onClick={() => handleDeleteSearch(search._id)} className={styles.deleteButton}>
                                        <Icon name="trash" />
                                    </button>
                                </li>
                            ))
