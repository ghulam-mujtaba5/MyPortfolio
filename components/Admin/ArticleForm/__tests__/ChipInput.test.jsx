...
describe("ChipInput", () => {
  ...
  test("keyboard reordering with Alt+Arrow changes order", async () => {
    const user = userEvent.setup();
    render(
      <Harness initial="alpha, beta, gamma" props={{ reorderable: true }} />,
    );
  ...
  test("remove button removes the specific chip", async () => {
    ...
    const alphaChip = within(chipContainer)
      .getByText("alpha")
      .closest("span[draggable]");

    // Find the remove button specifically within that chip
    const alphaRemove = within(alphaChip).getByRole("button", {
      name: /remove item: alpha/i,
    });
